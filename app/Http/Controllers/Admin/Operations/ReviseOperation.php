...
trait ReviseOperation
{
    ...
    public function listRevisions($id)
    {
        ...
        foreach ($revisions as $date => $changes) {
            foreach ($changes as $index => $change) {
                if ($change->key === 'content') {
                    $old = json_decode($change->old_value, true);
                    $new = json_decode($change->new_value, true);
                    ...
                    foreach ($new as $key => $value) {
                        ...
                        if ($old[$key] != $value) {
                            $newChange = new Revision;

                            $newChange->key = $key;
                            $newChange->new_value = $value;
                            $newChange->old_value = $old[$key];
                            $newChange->revisionable_type = $change->revisionable_type;
                            $newChange->user_id = $change->user_id;

                            $changes[] = $newChange;
                        }
                    }

                    // remove $content changed;
